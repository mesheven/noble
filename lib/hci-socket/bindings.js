...
NobleBindings.prototype.connect = function (peripheralUuid, parameters) {
  ...
  // stored
  this._dataLength[peripheralUuid] = parameters.dataLength;
...
NobleBindings.prototype.init = function () {
  ...
  // set Data length 
  this._hci.on('remoteFeatureComplete', this.onRemoteFeatureComplete.bind(this));
  this._hci.on('leDataLengthUpdateComplete', this.onLeDataLengthComplete.bind(this));
...
NobleBindings.prototype.onDiscover = function (status, address, addressType, connectable, advertisement, rssi) {
  ...
  if (hasScanServiceUuids) {
    ...
    this._dataLength[uuid]  = undefined;
...
NobleBindings.prototype.onLeConnComplete = function (status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  if (status === 0) {
    ...
    this._hci.readRemoteFeature(handle);
...
NobleBindings.prototype.onLeDataLengthComplete = function(handle, maxTxOctets, maxRxOctets, maxTXTime, maxRXTime) {
  const uuid = this._handles[handle];
  // exchange MTU size
  this._gatts[handle].exchangeMtu(256);
...
NobleBindings.prototype.onRemoteFeatureComplete = function(handle, featureLow, featureHigh) {
  const uuid = this._handles[handle];
  if ((featureLow & 0x20) != 0x00) {
    // support data length ext. bit 5
    if (this._dataLength[uuid] != undefined && this._dataLength[uuid] != 27) {
      this._hci.setDataLength(handle, this._dataLength[uuid], 0x0848);
      return;
    }
  }
  // exchange MTU size
