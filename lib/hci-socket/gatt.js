...
Gatt.prototype.onAclStreamData = function (cid, data) {
  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    const valueData = Buffer.from(data.subarray(3));
    ...
    if (opcode === ATT_OP_READ_BY_GROUP_RESP) {
      ...
      for (i = 0; i < num; i++) {
        ...
        services.push({
          ...
          uuid: (type === 6) ? data.readUInt16LE(offset + 4).toString(16) : data.subarray(offset + 4).subarray(0, 16).toString('hex').match(/.{1,2}/g).reverse().join('')
        });
      }
    }
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      for (i = 0; i < num; i++) {
        ...
        includedServices.push({
          ...
          uuid: (type === 8) ? data.readUInt16LE(offset + 6).toString(16) : data.subarray(offset + 6).subarray(0, 16).toString('hex').match(/.{1,2}/g).reverse().join('')
        });
      }
    }
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      for (i = 0; i < num; i++) {
        ...
        characteristics.push({
          ...
          uuid: (type === 7) ? data.readUInt16LE(offset + 5).toString(16) : data.subarray(offset + 5).subarray(0, 16).toString('hex').match(/.{1,2}/g).reverse().join('')
        });
      }
    }
...
Gatt.prototype.read = function (serviceUuid, characteristicUuid) {
  ...
  const callback = (data) => {
    ...
    if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      readData = Buffer.from(`${readData.toString('hex')}${data.subarray(1).toString('hex')}`, 'hex');
    ...
    if (opcode === ATT_OP_FIND_INFO_RESP) {
      ...
      for (i = 0; i < num; i++) {
        ...
        descriptors.push({
          ...
          uuid: (format === 0x01) ? data.readUInt16LE(offset + 2).toString(16) : data.subarray(offset + 2).subarray(0, 16).toString('hex').match(/.{1,2}/g).reverse().join('')
        });
      }
    }
