...
const EVT_LE_READ_REMOTE_FEATURE  = 0x04;
const EVT_LE_DATA_LENGTH_CHANGED  = 0x07;
...
const OCF_LE_READ_REMOTE_FEATURE = 0x0016;
...
const LE_READ_REMOTE_FEATURE_CMD = OCF_LE_READ_REMOTE_FEATURE | OGF_LE_CTL << 10;
...
//  const leEventMask = Buffer.from('1f00000000000000', 'hex');
  const leEventMask = Buffer.from('5f08000000000000', 'hex');
...
Hci.prototype.setDataLength = function(handle, size, maxTxTime) {
  ...
  // data
  cmd.writeUInt16LE(handle, 4); // handle
  cmd.writeUInt16LE(size, 6);   // TX size
  cmd.writeUInt16LE(maxTxTime, 8); // max TX time,

  debug('set data length - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};

Hci.prototype.readRemoteFeature = function(handle) {
  const cmd = Buffer.alloc(6);

  // header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(LE_READ_REMOTE_FEATURE_CMD, 1);
  ...
  debug('read Remote Feature - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
...
Hci.prototype.writeAclDataPkt = function (handle, cid, data) {
  ...
  data.copy(first, 9);
  data = data.subarray(first.length - 9);
  ...
  while (data.length > 0) {
    ...
    data.copy(frag, 5);
    data = data.subarray(frag.length - 5);
...
Hci.prototype.onSocketData = function (data) {
  ...
  if (HCI_EVENT_PKT === eventType) {
    ...
    if (subEventType === EVT_DISCONN_COMPLETE) {
      ...
      const result = Buffer.from(data.subarray(7));
      ...
      const leMetaEventData = Buffer.from(data.subarray(5));
    ...
    if (ACL_START === flags) {
      ...
      const pktData = Buffer.from(data.subarray(9));
...
Hci.prototype.processLeMetaEvent = function (eventType, status, data) {
  ...
  } else if (eventType === EVT_LE_READ_REMOTE_FEATURE) {
      this.processRemoteFeatureComplete(status, data);  
  } else if (eventType === EVT_LE_DATA_LENGTH_CHANGED) {
      this.processDataLengthUpdateComplete(status, data);  
  }
};
...
Hci.prototype.processLeConnComplete = function (status, data) {
  ...
  const address = data.subarray(4, 10).toString('hex').match(/.{1,2}/g).reverse().join(':');
...
Hci.prototype.processLeAdvertisingReport = function (count, data) {
  try {
    for (let i = 0; i < count; i++) {
      ...
      const address = data.subarray(2, 8).toString('hex').match(/.{1,2}/g).reverse().join(':');
      const eirLength = data.readUInt8(8);
      const eir = Buffer.from(data.subarray(9, eirLength + 9));
      ...
      data = data.subarray(eirLength + 10);
    }
  } catch (e) {
...
Hci.prototype.processDataLengthUpdateComplete = function(status, data) {
  const handle      = (data.readUInt8(0) << 8) | status ;
  ...
  debug(`\t\t\thandle      = ${handle}`);
  debug(`\t\t\tmaxTxOctets = ${maxTxOctets}`);
  debug(`\t\t\tmaxRxOctets = ${maxRxOctets}`);
  debug(`\t\t\tmaxTXTime   = ${maxTXTime}`);
  debug(`\t\t\tmaxRXTime   = ${maxRXTime}`);
  
  this.emit('leDataLengthUpdateComplete', handle, maxTxOctets, maxRxOctets, maxTXTime, maxRXTime);
};

Hci.prototype.processRemoteFeatureComplete = function(status, data) {
  const handle      = data.readUInt16LE(0);
  const featureLow  = data.readUInt32LE(2);
  const featureHigh = data.readUInt32LE(6);

  debug(`\t\t\thandle        = ${handle}`);
  debug(`\t\t\\tfeature low  mask = ${featureLow}`);
  debug(`\t\t\\tfeature High mask = ${featureHigh}`);
  
  this.emit('remoteFeatureComplete', handle, featureLow, featureHigh);
};
