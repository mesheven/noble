...
Gap.prototype.onHciLeAdvertisingReport = function (status, type, address, addressType, eir, rssi) {
  ...
  while ((i + 1) < eir.length) {
    ...
    const bytes = eir.subarray(i + 2).subarray(0, length - 1);
    ...
    switch (eirType) {
      ...
      case 0x07: // Complete List of 128-bit Service Class UUIDs
        for (let j = 0; j < bytes.length - 15; j += 16) {
          const serviceUuid = bytes.subarray(j, j + 16).toString('hex').match(/.{1,2}/g).reverse().join('');
          if (advertisement.serviceUuids.indexOf(serviceUuid) === -1) {
            advertisement.serviceUuids.push(serviceUuid);
          }
        }
      ...
      case 0x15: // List of 128 bit solicitation UUIDs
        for (let j = 0; j < bytes.length - 15; j += 16) {
          const serviceSolicitationUuid = bytes.subarray(j, j + 16).toString('hex').match(/.{1,2}/g).reverse().join('');
          if (advertisement.serviceSolicitationUuids.indexOf(serviceSolicitationUuid) === -1) {
            advertisement.serviceSolicitationUuids.push(serviceSolicitationUuid);
          }
        }
      ...
      case 0x16: // 16-bit Service Data, there can be multiple occurences
        advertisement.serviceData.push({
          uuid: bytes.subarray(0, 2).toString('hex').match(/.{1,2}/g).reverse().join(''),
          data: Buffer.from(bytes.subarray(2, bytes.length))
        });
        break;

      case 0x20: // 32-bit Service Data, there can be multiple occurences
        advertisement.serviceData.push({
          uuid: bytes.subarray(0, 4).toString('hex').match(/.{1,2}/g).reverse().join(''),
          data: Buffer.from(bytes.subarray(4, bytes.length))
        });
        break;

      case 0x21: // 128-bit Service Data, there can be multiple occurences
        advertisement.serviceData.push({
          uuid: bytes.subarray(0, 16).toString('hex').match(/.{1,2}/g).reverse().join(''),
          data: Buffer.from(bytes.subarray(16, bytes.length))
        });
        break;
